(function () {
    ...
    /**
     * Expose `parser`. Defined below, and used to parse a given
     * (english) string into an object model.
     */
    ...
    function tokenizeParagraph(paragraph, value) {
        ...
        while (sentence = sentences[++iterator]) {
            ...
            if (sentence[1]) {
                paragraph.append(new TextOM.WhiteSpaceNode(sentence[1]));
            }

            tokenizeSentence(paragraph.append(
                new TextOM.SentenceNode()), sentence[2]
            );
        }
    ...
    function tokenizeSentence(sentence, value) {
        ...
        while (token = tokens[++iterator]) {
            ...
            if (token.match(EXPRESSION_WHITE_SPACE)) {
                sentence.append(new TextOM.WhiteSpaceNode(token));
            } else if (token.match(EXPRESSION_WORD_MULTIPUNCTUATION)) {
                sentence.append(new TextOM.PunctuationNode(token));
            } else {
                sentence.append(new TextOM.WordNode(token));
            }
        }
    ...
    /**
     * `constructors` is an array containing the `.+Node` constructors of
     * TextOM, accessible by their type.
     *
     * @global
     * @private
     * @constant
    ...
    /**
     * `types` is an array containing the `.+Node` names of TextOM,
     * accessible by their type.
     *
     * @global
     * @private
     * @constant
    ...
    types = (function () {
        ...
        constructors.forEach(function (Constructor) {
            result[Constructor.prototype.type] = Constructor.name;
        });
    ...
    function replaceContent(source) {
        ...
        if (!self || !(self instanceof TextOM.Parent ||
            self instanceof TextOM.Element)) {
                throw new TypeError('Type Error');
        }
    ...
    /**
     * Expose `TextOM`. Imported above.
     *
     * @api public
     * @memberof parser
     * @constructor
     */
    exports.TextOM = TextOM;
})();
